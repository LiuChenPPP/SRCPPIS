...
def train(model, train_dataframe, valid_dataframe, args, fold=0):
    if args.dataset == '737':
        ...
        train_loader = DataLoader(dataset=PPI_Data(config.data_1082, train_dataframe['ID'].values),
                                  batch_size=config.BATCH_SIZE, shuffle=True, num_workers=2)
        valid_loader = DataLoader(dataset=PPI_Data(config.data_1082, valid_dataframe['ID'].values),
                                  batch_size=config.BATCH_SIZE, shuffle=True, num_workers=2)
...
def cross_validation(all_dataframe, args, fold_number=5):
    ...
    # result_test['binary_acc'], result_test['precision'], result_test['recall'], result_test['f1'], result_test['AUC'], \
    # result_test['AUPRC'], result_test['mcc']
    for f in range(fold):
    ...
    print(
        "Final! Test! Test_ACC={}±{},Test_Precision={}±{},Test_Recall={}±{},Test_F1={}±{},Test_AUC={}±{},Test_AUPRC={}±{}, Test_MCC={}±{},Test_Threshold={}".format(
            ...
            sum(Test_Recall) / len(Test_Recall), np.std(Test_Recall), sum(Test_F1) / len(Test_F1), np.std(Test_Recall),
...
def main():
    ...
    if args.dataset == '737':
        # train_data=PPI_Data(config.data_732,config.data_732_train,config.data_732_test,True)
        # test_data = PPI_Data(config.data_732, config.data_732_train, config.data_732_test, False)
        with open(config.data_732, 'rb') as f:
            Dset = pickle.load(f)
        f.close()
    elif args.dataset == '1082':
        with open(config.data_1082, 'rb') as f:
            Dset = pickle.load(f)
        f.close()
    ...
    aver_epoch = cross_validation(train_dataframe, args, fold_number=5)
