...
class Self_Attention(nn.Module):
    def __init__(self, hidden_size, num_attention_heads=4, num_neighbor=15, drop_rate=0):
        super().__init__()
    ...
    def forward(self, q, k, v, attention_mask=None, attention_weight=None, use_top=True):
        ...
        return outputs
class GTM(nn.Module):
    def __init__(self, protein_in_dim, protein_out_dim=256, target_dim=2, fc_layer_num=2, atten_layer_num=2,
                 atten_head=4, num_neighbor=20, drop_rate1=0.2, drop_rate2=0):
        super().__init__()
        ...
        # self.graph=Graph_extract(layers=1,hidden_dim=256,alpha=None,theta=None)
        self.hidden_block = []
        ...
        self.layers = nn.ModuleList(
            [Self_Attention(protein_out_dim, atten_head, num_neighbor, drop_rate2) for _ in range(atten_layer_num)])
    ...
    def forward(self, protein_node_features, protein_dist_matrix, protein_masks):
        ...
        for layer in self.layers:
            protein_embedding = layer(protein_embedding, protein_embedding, protein_embedding,
                                      protein_masks.unsqueeze(1).unsqueeze(1),
                                      dist_weight.unsqueeze(1))
